    ...
    export default {
        ...
        methods: {
            init() {
                ...
                this.itemsArr = this.$children.filter(item => item.$options.name === 'tf-slider-item');
            ...
            touchMoveHandler(event) {
                ...
                const touches = this.touches;
                if (!this.supportTouch || this.isVertical) {
                    let _diffX = Math.abs((event.touches ? event.touches[0].clientX : event.clientX) - touches.startX )
                    let _diffY = Math.abs((event.touches ? event.touches[0].clientY : event.clientY) - touches.startY )
                    // console.log(_diffX,_diffY,_diffX >_diffY)
                    if(this.isVertical){
                        if( _diffY >_diffX ) event.preventDefault();
                    }else{
                        if( _diffX >_diffY ) event.preventDefault();
                    }
                    // event.preventDefault();
                }
